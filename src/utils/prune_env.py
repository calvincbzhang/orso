# Code from https://github.com/eureka-research/Eureka/blob/main/eureka/utils/prune_env.py
...
def modify_python_file(task, filename, output):
    ...
    for line in lines:
        ...
        # Keep comments indicate parts to not modify
        if "# do not modify from here ..." in line or "# ... to here" in line:
...
def prune_python_class(
    filename, output, methods_to_keep, new_docstring, methods_to_prune_docstring
):
    with open(filename, "r") as file:
        lines = file.readlines()
...
def create_yaml(file_path, yaml_path):
    ...
    data = {"task": class_name, "env_name": env_name, "description": description}

    with open(yaml_path, "w") as outfile:
        yaml.dump(data, outfile, default_flow_style=False)


def prune(task, methods_to_keep = [], reward_name=None, new_name=""):
    ...
    methods_to_keep.extend([
        "compute_observations",
        f"compute_{task}_observations",
        "_update_states",
        f"compute_{task}_observations_states",
        "compute_full_state"
    ])

    # Create base environment file to write reward function for
    modify_python_file(
        reward_name, f"{ISAAC_ROOT_DIR}/tasks/{task}.py", f"envs/{new_name}.py"
    )

    # Create a condensed version to serve as input to Eureka
    prune_python_class(
        f"envs/{new_name}.py",
        f"envs/{new_name}_obs.py",
        methods_to_keep,
        "Rest of the environment definition omitted.",
        [f"compute_{task}_reward"],
    )


def prune_eureka(task, methods_to_keep = [], reward_name=None, new_name=""):
    if reward_name is None:
        if "hand" in task:
            reward_name = "hand"
        elif "franka" in task:
            reward_name = "franka"
        elif "ball_balance" in task:
            reward_name = "bbot"
        else:
            reward_name = task
    ...
    prune_python_class(
        f"envs/{new_name}.py",
        f"envs/{new_name}_eureka_obs.py",
        methods_to_keep,
        "Rest of the environment definition omitted.",
        [f"compute_{task}_reward"],
    )